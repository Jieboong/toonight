...
filename = "kakao_novel_complete.csv"

result = csv.writer(open(filename, "w"))
result.writerow(("serial", "url", "pic", "genre", "view", "like", "on_serial",  "title", "story_writer", "illustrator", "publisher", "story_line", "key_words", "best_comments"))

driver.get('https://webtoon.kakao.com/original-novel?tab=complete')
...
for elem in elements :
    ...
    result.writerow((
        serial, url, pic, genre, view, like, on_serial, title, story_writer, illustrator, publisher, story_line, key_words, best_comments
    ))
    print(title)
    driver.back()
    time.sleep(0.5)
...
try :
	s3.meta.client.upload_file(filename, BUCKET_NAME, 'kakao/novel/'+datetime.datetime.strftime(datetime.date.today(),'%Y-%m-%d' )+'.csv', ExtraArgs={
		'ACL' : 'public-read'
	})
	result = True
except : 
